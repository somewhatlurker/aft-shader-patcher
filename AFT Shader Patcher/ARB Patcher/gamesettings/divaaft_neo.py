# gane-specific settings for diva arcade future tone (new skinning method)
GAME_NAME = "Project DIVA Arcade: Future Tone (Neo Skinning)"

from re import compile as recompile

# used for "addressing" skinning buffer (which is a texture under novidia)
# current texture size and offset info are passed in local 30
skinning_pixel_size_local = "program.local[30].x" # size of one pixel (eg. 1/767)
skinning_offset_local = "program.local[30].y" # offset of current mesh's data in texture
...
# coordinate is from original index, multiplied to correct pixel, then offset added
VP_SKINNING_REGEX_2 = recompile(r"(BUFFER4[\s\S]*?)CVT(?:.S32.F32)?\s*?(.*?),\s*?(.*?);")
VP_SKINNING_SUB_2 = f"\\1MAD \\2, \\3, {skinning_pixel_size_local}, {skinning_offset_local};"
